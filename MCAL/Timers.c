...
#include "Timers.h"
/*************************Pointer to functions to be assigned to ISR TIMER0*********************************/
...
/*************************Pointer to functions to be assigned to ISR TIMER1*********************************/
...
void TIMER0_Init(Timer0Mode_type mode,Timer0Scaler_type scaler)
{
	switch (mode)
	{
		...
		CLRAR_BIT(TCCR0,WGM00);
		CLRAR_BIT(TCCR0,WGM01);
		break;
...
void TIMER0_OC0Mode(OC0Mode_type mode)
{
	switch (mode)
	{
		...
		CLRAR_BIT(TCCR0,COM00);
		CLRAR_BIT(TCCR0,COM01);
		break;
		case OC0_TOGGLE:
		SET_BIT(TCCR0,COM00);
		CLRAR_BIT(TCCR0,COM01);
...
void Timer1_Init( Timer1Mode_type mode,Timer1Scaler_type scaler)
{
	switch (mode)
	{
		...
		CLRAR_BIT(TCCR1A,WGM10);
		CLRAR_BIT(TCCR1A,WGM11);
		CLRAR_BIT(TCCR1B,WGM12);
		CLRAR_BIT(TCCR1B,WGM13);
		break;
...
void Timer1_OCRA1Mode(OC1A_Mode_type oc1a_mode)
{
	switch (oc1a_mode)
	{
		...
		CLRAR_BIT(TCCR1A,COM1A0);
		CLRAR_BIT(TCCR1A,COM1A1);
		break;
		case OCRA_TOGGLE:
...
void Timer1_OCRB1Mode(OC1B_Mode_type oc1b_mode)
{
	switch (oc1b_mode)
	{
		...
		CLRAR_BIT(TCCR1A,COM1B0);
		CLRAR_BIT(TCCR1A,COM1B1);
		break;
		case OCRB_TOGGLE:
